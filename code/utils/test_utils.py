...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    scores, boxes, im_scale, blob_conv, cls_scores = im_detect_bbox_aug(model, im, box_proposals)
    timers['im_detect_bbox'].toc()

    return {'scores':scores, 'boxes':boxes, 'cls_scores':cls_scores}
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    return_dict = model(torch.Tensor(inputs['data']).cuda(), 
                        torch.Tensor(inputs['rois']).cuda(),
                        torch.Tensor(inputs['labels']).cuda())
    # return_dict = model(**inputs)


    if 'distillation_score' in return_dict:
        # print("pickle rick")
        ...
        scores += return_dict['distillation_score'].data.cpu().numpy().squeeze()
        
        scores /= cfg.REFINE_TIMES + 1
    ...
    elif return_dict['refine_score'] is not None:
        # print("oh no morty")
        scores = return_dict['refine_score'][0].data.cpu().numpy().squeeze()
        for i in range(1, cfg.REFINE_TIMES):
            scores += return_dict['refine_score'][i].data.cpu().numpy().squeeze()
    ...
    return scores, pred_boxes, im_scale, return_dict['blob_conv'], return_dict['cls_score'].cpu().numpy()
...
def im_detect_bbox_aug(model, im, box_proposals=None):
    ...
    def add_preds_t(scores_t, boxes_t, cls_t):
    ...
    if cfg.TEST.BBOX_AUG.H_FLIP:
        ...
        scores_hf, boxes_hf, _, cls_score = im_detect_bbox_hflip(model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals=box_proposals)
        
        add_preds_t(scores_hf, boxes_hf, cls_score)

    # Compute detections at different scales
    for scale in cfg.TEST.BBOX_AUG.SCALES:
        max_size = cfg.TEST.BBOX_AUG.MAX_SIZE
        scores_scl, boxes_scl, cls_score = im_detect_bbox_scale(model, im, scale, max_size, box_proposals)
        add_preds_t(scores_scl, boxes_scl, cls_score)

        if cfg.TEST.BBOX_AUG.SCALE_H_FLIP:
            scores_scl_hf, boxes_scl_hf, cls_score = im_detect_bbox_scale(model, im, scale, max_size, box_proposals, hflip=True)
            add_preds_t(scores_scl_hf, boxes_scl_hf, cls_score)

    # Perform detection at different aspect ratios
    for aspect_ratio in cfg.TEST.BBOX_AUG.ASPECT_RATIOS:
        scores_ar, boxes_ar, cls_score = im_detect_bbox_aspect_ratio(model, im, aspect_ratio, box_proposals)
        add_preds_t(scores_ar, boxes_ar, cls_score)

        if cfg.TEST.BBOX_AUG.ASPECT_RATIO_H_FLIP:
            scores_ar_hf, boxes_ar_hf, cls_score = im_detect_bbox_aspect_ratio(model, im, aspect_ratio, box_proposals, hflip=True)
            add_preds_t(scores_ar_hf, boxes_ar_hf, cls_score)

    # Compute detections for the original image (identity transform) last to
    # ensure that the Caffe2 workspace is populated with blobs corresponding
    # to the original image on return (postcondition of im_detect_bbox)
    scores_i, boxes_i, im_scale_i, blob_conv_i, cls_score_i = im_detect_bbox(model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, boxes=box_proposals)

    add_preds_t(scores_i, boxes_i, cls_score_i)
    ...
    if cfg.TEST.BBOX_AUG.SCORE_HEUR == 'ID':
        ...
        if scores_ts[0] is not None:
            ...
            scores_c = None
        cls_scores = np.mean(cls_ts, axis=0)
    ...
    return scores_c, boxes_c, im_scale_i, blob_conv_i, cls_scores
...
def im_detect_bbox_hflip(
    ...
    scores_hf, boxes_hf, im_scale, _, cls_score = im_detect_bbox(
    ...
    return scores_hf, boxes_inv, im_scale, cls_score
...
def im_detect_bbox_scale(
    ...
    """
    if hflip:
        scores_scl, boxes_scl, _, cls_score = im_detect_bbox_hflip(
            model, im, target_scale, target_max_size, box_proposals=box_proposals
        )
    else:
        scores_scl, boxes_scl, _, _, cls_score = im_detect_bbox(
            model, im, target_scale, target_max_size, boxes=box_proposals
        )
    return scores_scl, boxes_scl, cls_score
...
def im_detect_bbox_aspect_ratio(
    ...
    if hflip:
        scores_ar, boxes_ar, _, cls_score = im_detect_bbox_hflip(
            model,
            im_ar,
            cfg.TEST.SCALE,
            cfg.TEST.MAX_SIZE,
        ...
        scores_ar, boxes_ar, _, _, cls_score = im_detect_bbox(
            model,
            im_ar,
            cfg.TEST.SCALE,
            cfg.TEST.MAX_SIZE,
    ...
    return scores_ar, boxes_inv, cls_score


def box_results_for_corloc(scores, boxes, classy):  # NOTE: support single-batch
...
def box_results_with_nms_and_limit(scores, boxes, cls_scores):
