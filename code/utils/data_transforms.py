...
__all__ = ["Compose", "ToTensor", "ToCVImage",
		   "Normalize", "PaddedResize",  "Resize", "CenterCrop", "Pad",
		   "Lambda", "RandomApply", "RandomOrder", "RandomChoice", "RandomCrop",
		   "HorizontalFlip", "RandomVerticalFlip", "RandomResizedCrop",
...
class Lambda(object):
	...
	def __call__(self, to_transform):
		return self.lambd(to_transform)
...
class Resize(object):
	...
	def __call__(self, to_transform):
		...
		im_scale = float(to_transform['img_scale']) / float(im_size_max)
		to_transform['img'] = cv2.resize(to_transform['img'], None, None, fx=im_scale, fy=im_scale, interpolation=self.interpolation)
...
class PaddedResize(object):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (np.ndarray): Image to be scaled.

		Returns:
			np.ndarray: Rescaled image.
		"""
		size = self.size
		if 'img_scale' in to_transform:
			size = to_transform['img_scale']


		img_size = to_transform['img'].shape[:2]
		to_transform['img'] = F.padded_resize(to_transform['img'], size, self.interpolation)
		img_new_size = to_transform['img'].shape[:2]

		#Reshape the proposals to the new resolution 
		proposals = to_transform['proposals'].copy()

		proposals[:, 0::2] = proposals[:, 0::2]/img_size[1]*img_new_size[1]
		proposals[:, 1::3] = proposals[:, 1::3]/img_size[0]*img_new_size[0]

		to_transform['proposals'] = proposals

		# cv2.imshow( "img",  to_transform['img'])
		# cv2.waitKey(0)
...
class CenterCrop(object):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (CV Image): Image to be cropped.

		Returns:
			CV Image: Cropped image.
		"""
		return F.center_crop(to_transform, self.size)
...
class Pad(object):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (CV Image): Image to be padded.

		Returns:
			CV Image: Padded image.
		"""
		return F.pad(to_transform, self.padding, self.fill, self.padding_mode)
...
class RandomOrder(RandomTransforms):
	...
	def __call__(self, to_transform):
		...
		for i in order:
			to_transform = self.transforms[i](to_transform)
		return to_transform
...
class RandomCrop(object):
	...
	@staticmethod
	def get_params(to_transform, output_size):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (np.ndarray): Image to be cropped.

		Returns:
			np.ndarray: Cropped image.
		"""
		if self.padding > 0:
			to_transform = F.pad(to_transform, self.padding)

		# pad the width if needed
		if self.pad_if_needed and to_transform.shape[1] < self.size[1]:
			to_transform = F.pad(to_transform, (int((1 + self.size[1] - to_transform.shape[1]) / 2), 0))
		# pad the height if needed
		if self.pad_if_needed and to_transform.shape[0] < self.size[0]:
			to_transform = F.pad(to_transform, (0, int((1 + self.size[0] - to_transform.shape[0]) / 2)))

		i, j, h, w = self.get_params(to_transform, self.size)

		return F.crop(to_transform, i, j, h, w)
...
class HorizontalFlip(object):
	def __call__(self, to_transform):
		...
		if  to_transform['hflip']:
			to_transform['img'] = to_transform['img'][:, ::-1, :]
			
			im_width = to_transform['img'].shape[1]
			to_transform['proposals'] = box_utils.flip_boxes(to_transform['proposals'], im_width)
...
class RandomVerticalFlip(object):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (CV Image): Image to be flipped.

		Returns:
			CV Image: Randomly flipped image.
		"""
		if random.random() < self.p:
			return F.vflip(to_transform)
		return to_transform
...
class RandomResizedCrop(object):
	...
	@staticmethod
	def get_params(to_transform, scale, ratio):
		...
		for attempt in range(10):
			area = to_transform.shape[0] * to_transform.shape[1]
			...
			if w <= to_transform.shape[1] and h <= to_transform.shape[0]:
				i = random.randint(0, to_transform.shape[0] - h)
				j = random.randint(0, to_transform.shape[1] - w)
				return i, j, h, w

		# Fallback
		w = min(to_transform.shape[0], to_transform.shape[1])
		i = (to_transform.shape[0] - w) // 2
		j = (to_transform.shape[1] - w) // 2
		return i, j, w, w

	def __call__(self, to_transform):
		"""
		Args:
			to_transform (np.ndarray): Image to be cropped and resized.

		Returns:
			np.ndarray: Randomly cropped and resized image.
		"""
		i, j, h, w = self.get_params(to_transform, self.scale, self.ratio)
		return F.resized_crop(to_transform, i, j, h, w, self.size, self.interpolation)
...
class FiveCrop(object):
	...
	def __call__(self, to_transform):
		return F.five_crop(to_transform, self.size)
...
class TenCrop(object):
	...
	def __call__(self, to_transform):
		return F.ten_crop(to_transform, self.size, self.vertical_flip)
...
class ColorJitter(object):
	...
	def get_params(brightness, contrast, saturation, hue):
		...
		if brightness > 0:
			...
			transforms.append(Lambda(lambda to_transform: F.adjust_brightness(to_transform, brightness_factor)))

		if contrast > 0:
			contrast_factor = random.uniform(max(0, 1 - contrast), 1 + contrast)
			transforms.append(Lambda(lambda to_transform: F.adjust_contrast(to_transform, contrast_factor)))

		if saturation > 0:
			saturation_factor = random.uniform(max(0, 1 - saturation), 1 + saturation)
			transforms.append(Lambda(lambda to_transform: F.adjust_saturation(to_transform, saturation_factor)))

		if hue > 0:
			hue_factor = random.uniform(-hue, hue)
			transforms.append(Lambda(lambda to_transform: F.adjust_hue(to_transform, hue_factor)))
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (np.ndarray): Input image.
...
class RandomRotation(object):
	...
	def __call__(self, to_transform):
		"""
			to_transform (np.ndarray): Image to be rotated.
		...
		return F.rotate(to_transform, angle, self.resample, self.expand, self.center)
...
class RandomAffine(object):
	...
	Args:
		...
		translate (tuple, optional): tuple of maximum absolute fraction for horizontal
			...
			is randomly sampled in the range -to_transform_width * a < dx < to_transform_width * a and vertical shift is
			randomly sampled in the range -to_transform_height * b < dy < to_transform_height * b. Will not translate by default.
	...
	@staticmethod
	def get_params(degrees, translate, scale_ranges, shears, to_transform_size):
		...
		angle = random.uniform(degrees[0], degrees[1])
		if translate is not None:
			max_dx = translate[0] * to_transform_size[1]
			max_dy = translate[1] * to_transform_size[0]
	...
	def __call__(self, to_transform):
		"""
			to_transform (CV Image): Image to be transformed.

		Returns:
			CV Image: Affine transformed image.
		"""
		ret = self.get_params(self.degrees, self.translate, self.scale, self.shear, to_transform.shape)
		return F.affine(to_transform, *ret, resample=self.resample, fillcolor=self.fillcolor)
...
class Grayscale(object):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (CV Image): Image to be converted to grayscale.

		Returns:
			CV Image: Randomly grayscaled image.
		"""
		return F.to_grayscale(to_transform, num_output_channels=self.num_output_channels)
...
class RandomGrayscale(object):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (np.ndarray): Image to be converted to grayscale.
		...
		if random.random() < self.p:
			return F.to_grayscale(to_transform, num_output_channels=num_output_channels)
		return to_transform
...
class RandomPerspective(object):
	...
	def get_params(fov_range, anglex_ranges, angley_ranges, anglez_ranges, shear_ranges,
				   translate, scale_ranges,  to_transform_size):
	...
	def __call__(self, to_transform):
		"""
			to_transform (np.ndarray): Image to be transformed.

		Returns:
			np.ndarray: Affine transformed image.
		"""
		ret = self.get_params(self.fov, self.anglex, self.angley, self.anglez, self.shear,
							  self.translate, self.scale, to_transform.shape)
		return F.perspective(to_transform, *ret, resample=self.resample, fillcolor=self.fillcolor)
...
class RandomAffine6(object):
	...
	@staticmethod
	def get_params(to_transform_size, anglez_range=(0, 0), shear_range=(0, 0),
	...
	def __call__(self, to_transform):
		...
		Returns:
			np.ndarray: Affine transformed image.
		"""
		ret = self.get_params(to_transform.shape, self.anglez, self.shear, self.translate, self.scale)
		return F.affine6(to_transform, *ret, resample=self.resample, fillcolor=self.fillcolor)
...
class RandomGaussianNoise(object):
	...
	def __call__(self, to_transform):
		"""
		Args:
			to_transform (np.ndarray): Image to be noised.

		Returns:
			np.ndarray: Randomly noised image.
		"""
		if random.random() < self.p:
			return F.gaussian_noise(to_transform, mean=self.mean, std=self.std)
		return to_transform
...
class RandomPoissonNoise(object):
	...
	def __call__(self, to_transform):
		...
		"""
		if random.random() < self.p:
			return F.poisson_noise(to_transform)
		return to_transform
...
class RandomSPNoise(object):
	...
	def __call__(self, to_transform):
		...
		"""
		if random.random() < self.p:
			return F.salt_and_pepper(to_transform, self.prob)
		return to_transform
